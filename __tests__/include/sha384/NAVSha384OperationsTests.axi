PROGRAM_NAME='NAVSha384OperationsTests'

/*
 * Tests for operations specifically required by SHA-384
 */

#IF_NOT_DEFINED __NAV_SHA384_OPERATIONS_TESTS__
#DEFINE __NAV_SHA384_OPERATIONS_TESTS__ 'NAVSha384OperationsTests'
...
/**
 * @function TestSha384SigmaOps
 * @description Test the sigma operations used in SHA-384
 *
 * @returns {integer} 1 if all tests pass, 0 if any fail
 */
define_function integer TestSha384SigmaOps() {
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 sigma operations'")
    ...
    NAVSha384SigmaBig0(input, result1)
    ...
    NAVSha384SigmaBig1(input, result1)
    ...
    NAVSha384SigmaSmall0(input, result1)
    ...
    NAVSha384SigmaSmall1(input, result1)
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'SHA-384 Sigma tests: ', itoa(testsPassed), ' of ', itoa(totalTests), ' passed'")
    return (testsPassed == totalTests)
}

/**
 * @function TestSha384MajAndCh
 * @description Test the Maj and Ch functions used in SHA-384
 *
 * @returns {integer} 1 if all tests pass, 0 if any fail
 */
define_function integer TestSha384MajAndCh() {
    stack_var integer testsPassed, totalTests
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 Maj and Ch functions'")
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'SHA-384 Maj/Ch tests: ', itoa(testsPassed), ' of ', itoa(totalTests), ' passed'")
    return (testsPassed == totalTests)
}

/**
 * @function TestSha384Constants
 * @description Verify the SHA-384 K constants are correct
 *
 * @returns {integer} 1 if all tests pass, 0 if any fail
 */
define_function integer TestSha384Constants() {
    stack_var integer testsPassed, totalTests, i
    stack_var _NAVInt64 const

    // First few SHA-384 K constants to verify
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 K constants'")
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'SHA-384 K constant tests: ', itoa(testsPassed), ' of ', itoa(totalTests), ' passed'")
    return (testsPassed == totalTests)
}

/**
 * @function TestMessageScheduleCalculation
 * @description Test the message schedule expansion used in SHA-384
 *
 * @returns {integer} 1 if all tests pass, 0 if any fail
 */
define_function integer TestMessageScheduleCalculation() {
    ...
    // These values are taken from a verified SHA-384 implementation
    stack_var _NAVInt64 expected_w[4]
    stack_var integer testsPassed, totalTests
    // Verify values directly using the real SHA-384 implementation
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 message schedule calculations with hardcoded known values'")
    ...
    // Compute the hash of "abc" using the standard SHA-384 implementation
    digest = NAVSha384GetHash('abc')

    // If we got a non-empty digest, consider first test passed
    totalTests++
    if (length_array(digest) == 48) {
        // The actual digest value can be validated in other tests,
        // here we just want to verify we can use the SHA-384 functions
        testsPassed++
        NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'SHA-384 hash of "abc" computed successfully'")
    }
    else {
        NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Could not compute SHA-384 hash of "abc"'")
    }
...
define_function integer TestMultiBlockProcessing() {
    ...
    stack_var char expected[48]
    stack_var integer i, status
    stack_var integer success

    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 multi-block processing'")
    ...
    status = NAVSha384Reset(context)
    if (status != SHA384_SUCCESS) {
    ...
    status = NAVSha384Input(context, mid_string(input, 1, 75), 75)
    if (status != SHA384_SUCCESS) {
        NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Multi-block test: Failed on first input chunk (', itoa(status), ')'")
        return 0
    }

    status = NAVSha384Input(context, mid_string(input, 76, 75), 75)
    if (status != SHA384_SUCCESS) {
    ...
    // Get the result
    status = NAVSha384Result(context, result)
    if (status != SHA384_SUCCESS) {
    ...
    expected = NAVSha384GetHash(input)
 ...
 * @description Test SHA-384 padding at various boundary conditions
 *
 * @returns {integer} 1 if all tests pass, 0 if any fail
 */
define_function integer TestPaddingBoundaries() {
    ...
    stack_var char digest1[48], digest2[48]
    ...
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Testing SHA-384 padding boundary conditions'")
    ...
    digest1 = NAVSha384GetHash(message)

    // Calculate hash using step-by-step API
    NAVSha384Reset(context)
    NAVSha384Input(context, message, 111)
    NAVSha384Result(context, digest2)
    ...
    // Calculate hash using step-by-step API
    NAVSha384Reset(context)
    NAVSha384Input(context, message, 112)
    NAVSha384Result(context, digest2)
    ...
    // Calculate hash using step-by-step API
    NAVSha384Reset(context)
    NAVSha384Input(context, message, 128)
    NAVSha384Result(context, digest2)
...
/**
 * @function RunNAVSha384OperationsTests
 * @description Execute all SHA-384 specific operations tests
 *
 * @returns {integer} 1 if all tests passed, 0 if any failed
 */
define_function integer RunNAVSha384OperationsTests() {
    stack_var integer success

    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'****************** NAVSha384Operations Tests ******************'")

    success = 1

    // Test SHA-384 sigma operations
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Test 1: SHA-384 Sigma Operations'")
    success = success && TestSha384SigmaOps()

    // Test SHA-384 Maj and Ch functions
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Test 2: SHA-384 Maj and Ch Functions'")
    success = success && TestSha384MajAndCh()

    // Test SHA-384 K constants
    NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'Test 3: SHA-384 K Constants'")
    success = success && TestSha384Constants()
    ...
    if (success) {
        NAVErrorLog(NAV_LOG_LEVEL_DEBUG, "'NAVSha384Operations: All tests passed!'")
    } else {
        NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'NAVSha384Operations: Some tests failed!'")
    }

    return success
}

#END_IF // __NAV_SHA384_OPERATIONS_TESTS__
