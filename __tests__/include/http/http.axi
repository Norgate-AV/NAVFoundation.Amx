...
define_function RunHttpTests() {
    ...
    for (x = 1; x <= length_array(HTTP_TEST); x++) {
        ...
        if (!NAVParseUrl(HTTP_TEST[x][2], url)) {
        ...
        if (result != HTTP_EXPECTED_RESULT[x]) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Expected init result "', NAVBooleanToString(HTTP_EXPECTED_RESULT[x]), '" but got "', NAVBooleanToString(result), '"'")
            continue
        }

        if (HTTP_EXPECTED_RESULT[x] && !result) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Failed to init request'")
            continue
        }

        if (request.Method != HTTP_TEST[x][1]) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Expected method "', HTTP_TEST[x][1], '" but got "', request.Method, '"'")
            continue
        }

        if (request.Host != HTTP_EXPECTED_HOST[x]) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Expected host "', HTTP_EXPECTED_HOST[x], '" but got "', request.Host, '"'")
            continue
        }

        if (request.Path != HTTP_EXPECTED_PATH[x]) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Expected path "', HTTP_EXPECTED_PATH[x], '" but got "', request.Path, '"'")
            continue
        }

        if (request.Body != HTTP_TEST[x][3]) {
            NAVErrorLog(NAV_LOG_LEVEL_ERROR, "'Test ', itoa(x), ' failed. Expected body "', HTTP_TEST[x][3], '" but got "', request.Body, '"'")
            continue
        }
        ...
        if (length_array(HTTP_TEST_HEADERS[x]) > 0) {
            ...
            for (z = 1; z <= length_array(HTTP_TEST_HEADERS[x]); z++) {
                if (!length_array(HTTP_TEST_HEADERS[x][z])) {
                    continue
                }
