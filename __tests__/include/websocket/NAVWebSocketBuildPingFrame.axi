PROGRAM_NAME='NAVWebSocketBuildPingFrame'
...
constant char WEBSOCKET_BUILDPINGFRAME_TEST_PAYLOAD[][130] = {
    'ping',                     // Test 1: Normal payload
    '',                         // Test 2: Empty payload
    {$01, $02, $03},           // Test 3: Binary payload
    'x',                        // Test 4: Single byte
...
constant char WEBSOCKET_BUILDPINGFRAME_TEST_MASKED[] = {
    true,   // Test 1: Masked
    true,   // Test 2: Masked
...
constant char WEBSOCKET_BUILDPINGFRAME_EXPECTED_RESULT[] = {
    true,   // Test 1: Valid
...
define_function TestNAVWebSocketBuildPingFrame() {
    stack_var integer x

    NAVLogTestSuiteStart("'NAVWebSocketBuildPingFrame'")

    for (x = 1; x <= length_array(WEBSOCKET_BUILDPINGFRAME_TEST_PAYLOAD); x++) {
        stack_var char output[1000]
        stack_var char result
        stack_var integer payloadLen

        result = NAVWebSocketBuildPingFrame(WEBSOCKET_BUILDPINGFRAME_TEST_PAYLOAD[x],
                                            WEBSOCKET_BUILDPINGFRAME_TEST_MASKED[x],
                                            output)

        if (!NAVAssertBooleanEqual('Build should match expected result',
                                   WEBSOCKET_BUILDPINGFRAME_EXPECTED_RESULT[x],
                                   result)) {
            NAVLogTestFailed(x,
                            itoa(WEBSOCKET_BUILDPINGFRAME_EXPECTED_RESULT[x]),
                            itoa(result))
            continue
        }

        if (!WEBSOCKET_BUILDPINGFRAME_EXPECTED_RESULT[x]) {
        ...
        // Verify opcode is PING (0x09) in first byte (lower 4 bits)
        if (!NAVAssertIntegerEqual('Opcode should be PING (0x09)',
                                            $09,
                                            output[1] band $0F)) {
            NAVLogTestFailed(x, itoa($09), itoa(output[1] band $0F))
            continue
        }
        ...
        if (!NAVAssertBooleanEqual('MASK bit should match masked parameter',
                                   ...
                                   (output[2] band $80) != 0)) {
            NAVLogTestFailed(x,
                            itoa(WEBSOCKET_BUILDPINGFRAME_TEST_MASKED[x]),
                            itoa((output[2] band $80) != 0))
            continue
        }

        // Verify payload length <= 125 for control frames
        payloadLen = length_array(WEBSOCKET_BUILDPINGFRAME_TEST_PAYLOAD[x])
    ...
    NAVLogTestSuiteEnd("'NAVWebSocketBuildPingFrame'")
}
