PROGRAM_NAME='NAVRegexCompileGroupErrors'
...
constant char REGEX_COMPILE_GROUP_ERRORS_PATTERN_TEST[][255] = {
...
DEFINE_FUNCTION TestNAVRegexCompileGroupErrors() {
    stack_var integer i

    NAVLog("'***************** NAVRegexCompile - Group Error Cases *****************'")

    for (i = 1; i <= length_array(REGEX_COMPILE_GROUP_ERRORS_PATTERN_TEST); i++) {
        stack_var _NAVRegexParser parser
        stack_var char result

        result = NAVRegexCompile(REGEX_COMPILE_GROUP_ERRORS_PATTERN_TEST[i], parser)
